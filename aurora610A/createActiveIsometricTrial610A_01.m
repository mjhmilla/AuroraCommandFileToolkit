function success = createActiveIsometricTrial610A_01(...
                    startEndLength,...
                    lengthDuringActivation,...
                    trialFileNameNoExt,...                                     
                    auroraConfig,...
                    expConfig,...
                    expFolders,...
    ...
    %
    % Ramp to lengthDuringActivation
    %
    lengthRampOptions = ...
      getCommandFunctionOptions610A('Ramp','Length Out',auroraConfig);

    positioningDuration = abs(lengthDuringActivation)...
                          /expConfig.positioning.rampSpeedInMMPS;
    positioningDuration = max(positioningDuration,...
                              expConfig.positioning.minimumRampTime);

    lengthRampOptions(1).value = lengthDuringActivation; 
    lengthRampOptions(2).value = positioningDuration;
    ...
    if(abs(lengthDuringActivation)>1e-6)
    ...
    %
    % Ramp to start/end length
    %
    lengthRampOptions = ...
      getCommandFunctionOptions610A('Ramp','Length Out',auroraConfig);

    positioningDuration = abs(startEndLength-lengthDuringActivation)...
                          /expConfig.positioning.rampSpeedInMMPS;
    positioningDuration = max(positioningDuration,...
                              expConfig.positioning.minimumRampTime);
    ...
    experimentTags = {'isometric-tetanus'};
