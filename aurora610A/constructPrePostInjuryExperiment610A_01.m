...
for idxPI = 1:1:length(expConfig.passive.impedance.length)
  ...
  expConfigRecoveryTrial=[];
  expConfigRecoveryTrial=expConfig.recovery;
  expConfigRecoveryTrial.muscle=expConfig.muscle;
  expConfigRecoveryTrial.positioning=expConfig.positioning;
  expConfigRecoveryTrial.timing=expConfig.timing;
...
for idxRamp = 1:1:size(expConfig.activeRamp.lengths,2)
  ...
  for idxActive=1:1:2
      ...
      expConfigRampTrial = [];
      expConfigRampTrial.tetanus = expConfig.tetanus;
      expConfigRampTrial.timing  = expConfig.timing;
      expConfigRampTrial.positioning = expConfig.positioning;
      expConfigRampTrial.muscle      = expConfig.muscle;

      expConfigRampTrial.ramp.length   = expConfig.activeRamp.lengths(idxRamp,2);
      expConfigRampTrial.ramp.duration = duration;
      expConfigRampTrial.ramp.isActive = isActive;
      expConfigRampTrial.ramp.holdDuration = 0;
      


      success = createRampTrial610A_01(...    
                      startEndLength,...
                      expConfig.activeRamp.lengths(idxRamp,1),...
                      trialFileNameNoExt,...                                     
                      auroraConfig.default,...
...
for idxIso = 1:1:length(expConfig.isometric.lengths)
  ...
  expConfigIsoTrial = [];
  expConfigIsoTrial.timing      = expConfig.timing;
  expConfigIsoTrial.tetanus     = expConfig.tetanus;  
  expConfigIsoTrial.positioning = expConfig.positioning;
  expConfigIsoTrial.muscle      = expConfig.muscle;

  expConfigIsoTrial.ramp.length   = expConfig.isometric.lengths(idxIso);
  expConfigIsoTrial.ramp.duration = duration;
  expConfigIsoTrial.ramp.isActive = isActive;
  expConfigIsoTrial.ramp.holdDuration = 0;

  success = createActiveIsometricTrial610A_01(...
                      startEndLength,...
