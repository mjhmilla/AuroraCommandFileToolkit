...
if(length(configVibration.frequencyRange) == 2 ...
    ...
    randomVecCUpd= randomVecC;

    randomSignals.frequency = ...
        (1-randomVecCUpd).*(configVibration.frequencyRange(1,1)) ...
           +randomVecCUpd.*(configVibration.frequencyRange(1,2)...
...
if(length(configVibration.magnitudeRange) == 2 ...
    ...
    randomSignals.magnitude = ...
      ones(size(randomVecD)).*configVibration.magnitudeRange(1,1);
end
...
switch controlFunctionName
    case 'Step' 
      ...
      [timeVec,signalVec,controlFunctions,lineCount] = ...
...
%            indexArg = [1:1:(3*length(stochasticFunctions.waitDuration))]';
%     
%            argScaleDur = 1/configVibration.holdRange(1,1);
%            argScaleVel = 1/configVibration.normSpeedRange(1,1);
%     
%            x0       = [randomSignals.duration(indexArg,1).*argScaleDur;...
%                        randomSignals.velocity(indexArg,1).*argScaleVel];
%     
%            argScale = [argScaleDur;argScaleVel];
%     
%            fcnWave = @(arg)createSquareWavePattern610A_FcnWrapper(...
%                              arg,...
%                              indexArg,...
%                              configVibration,...
%                              randomSignals.duration,...
...
%                                 objScale);
%     
%            lb = [ones(size(indexArg)).*(argScale(1,1)*configVibration.holdRange(1,1));...
%                  ones(size(indexArg)).*(argScale(2,1)*configVibration.normSpeedRange(1,1))];
%     
%            ub = [ones(size(indexArg)).*(argScale(1,1)*configVibration.holdRange(1,2));...
%                  ones(size(indexArg)).*(argScale(2,1)*configVibration.normSpeedRange(1,2))];
%     
%            [x1,resnorm,residual,exitflag] = ...
%              lsqnonlin(fcnWaveError,x0,lb,ub);
%     
%             x0=x1;       
%            spectrumError = calcWavePowerSpectrumError(...
%                               x0,fcnWave,...
...
%             j1 = length(x1);
%             
%             randomSignals.duration= x1(i0:i1,1)./argScale(1,1);
%             randomSignals.velocity= x1(j0:j1,1)./argScale(2,1);
%     
%             [timeVec,...
%             signalVec,...
%             stochasticFunctions,...
...
%         end
    ...
    case 'Sine Wave'
      ...
      if(configVibration.fitPerturbationPowerSpectrum==1 || flag_testing == 1)

         indexArg = [1:1:nCommands]';
    
         argScale = 1./configVibration.magnitudeRange(1,2);
    
         magnitudeDefault = ones(nCommands,1).*configVibration.magnitudeRange(1,1);
         ...
         lb = ones(size(indexArg)).*0.01;    
         ub = ones(size(indexArg));
         ...
         for idxLoop=1:1:1
          ...
          randomSignals.magnitude= x1(i0:i1,1)./argScale(1,1);
